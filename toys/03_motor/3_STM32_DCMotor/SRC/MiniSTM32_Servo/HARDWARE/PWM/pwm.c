...
void TIM1_1PWM_Init(u16 arr,u16 psc)
{
    ...
    TIM_CtrlPWMOutputs(TIM1,ENABLE);	//MOE 主输出使能(Tim1必须要做这一步)
...
void TIM3_PWM_Init(u16 arr,u16 psc)
{
    ...
    //设置该引脚为复用输出功能,输出TIM1 CH1的PWM脉冲波形
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5; //TIM3_CH1->PB5
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;  //复用推挽输出
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(GPIOB, &GPIO_InitStructure);

    GPIO_PinRemapConfig(GPIO_PartialRemap_TIM3, ENABLE); // Tim3_CH2的PB5 部分重映射(Tim3_CH1对应的是PB4)
    ...
    TIM_OCInitStructure.TIM_Pulse = 0; //设置待装入捕获比较寄存器的脉冲值
    TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High; //输出极性:TIM输出比较极性高
    TIM_OC2Init(TIM3, &TIM_OCInitStructure);  //根据TIM_OCInitStruct中指定的参数初始化外设TIMx(Tim3的通道2：OC2Init)

    TIM_OC2PreloadConfig(TIM3, TIM_OCPreload_Enable);  //CH2预装载使能(通道2)
